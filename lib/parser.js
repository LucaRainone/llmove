...
function parseContent(files) {
  ...
  for (const file of files) {
    ...
    const systemMatch = content.match(/^\s*<system[^>]*>([\s\S]*)<\/system>/i);
    const promptMatch = content.match(/^\s*<prompt[^>]*>([\s\S]*)<\/prompt>/i);

    if (systemMatch) system += systemMatch[1].trim() + '\n';
    if (promptMatch) prompt += promptMatch[1].trim().replace(/^\s*<system[^>]*>([\s\S]*?)<\/system>/i, "") + '\n';
  }
